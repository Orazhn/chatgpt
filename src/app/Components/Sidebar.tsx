...
interface MessagesComponentProps {
  ...
  const Sidebar: FC<MessagesComponentProps> = ({allMessages}) => {
    ...
    <div>
      ...
      <motion.aside 
        id="default-sidebar" 
        className={`top-0 left-0 z-40 sm:w-2/3 md:w-1/3 lg:w-1/6 h-full transition-transform sm:translate-x-0 sm:fixed 2xl:w-full overflow-y-auto`} 
        aria-label="Sidebar" 
        ...
        <div className="h-full px-3 py-4 overflow-y-hidden bg-gray-50 dark:bg-gray-800 2xl:w-96">
          <ul className="space-y-2 font-medium">
            <li className=" w-full flex justify-between items-center" >
                ...
                <motion.button  whileHover={{ scale: 1.2 }} onClick={() => setSideOpen(false)} className=" text-gray-900 rounded-lg  dark:text-white">
